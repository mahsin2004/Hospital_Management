...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($result->num_rows > 0) {
        ...
        if (md5($password) === $user['password']) { // Again, use bcrypt in production
...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
    <div class="bg-white p-6 rounded shadow-md w-96">
        ...
        <form action="login.php" method="POST">
            ...
            <?php endif; ?>
            <button type="submit" class="w-full bg-indigo-500 text-white py-2 px-4 rounded-md hover:bg-indigo-600">Login</button>
        </form>
        <div class="mt-4 text-center">
            <p class="text-sm">Not registered yet? <a href="register.php" class="text-indigo-500 hover:text-indigo-700">Register</a></p>
        </div>
    </div>
</body>
